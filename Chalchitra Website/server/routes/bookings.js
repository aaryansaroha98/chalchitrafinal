              ...
              QRCode.toDataURL(JSON.stringify(qrData), (qrErr, qrDataURL) => {
                ...
                db.run('UPDATE bookings SET qr_code = ? WHERE id = ?',
                  [qrDataURL, databaseId], function(updateErr) {
                    ...
                    // Increment coupon usage count if coupon was used and it's a paid booking
                    // (Free bookings don't count as coupon usage since no payment was made)
                    if (coupon_code && req.body.payment_id !== 'FREE_BOOKING') {
...
router.get('/check/:movieId', (req, res) => {
  // Temporarily allow checking without authentication for testing
  // if (!req.user) return res.status(401).json({ error: 'Authentication required' });
