...
<Card.Body className="text-center" style={{ position: 'relative', zIndex: 1 }}>
                  ...
                  <tr key={member.id}>
                    <td>
                      <div style={{
                        ...
                        {member.photo_url ? (
                          <img
                            src={member.photo_url.startsWith('/') ? member.photo_url : `/uploads/${member.photo_url}`}
                            alt={member.name}
                            style={{
                              width: '100%',
                              height: '100%',
              ...
              {gallery && gallery.length > 0 ? gallery.map((image, index) => (
                <Col key={image.id || index} md={3} className="mb-4">
                  <Card className="bg-glass border-glass text-white">
                    <Card.Img
                      variant="top"
                      src={image.image_url.startsWith('/') ? image.image_url : `/uploads/${image.image_url}`}
                      style={{
                        height: '200px',
                        objectFit: 'contain',
        ...
        {activeTab === 'feedback' && (
          <div className="text-center text-white py-5">
            <h2>User Feedback</h2>
            <p>View feedback and ratings from users.</p>
            <Table striped bordered hover className="mt-4">
              <thead>
                <tr>
                  <th>User</th>
              ...
              <tbody>
                {feedback && feedback.length > 0 ? feedback.map(item => (
                  <tr key={item.id}>
                    <td>{item.user_name || item.user_email || 'Anonymous'}</td>
                    <td>{item.movie_title || 'N/A'}</td>
                    <td>
                    ...
                    <Card style={{
                      ...
                      <Card.Body className="text-center d-flex flex-column justify-content-center">
                        ...
                        <Card.Text style={{ color: 'rgba(255, 255, 255, 0.7)' }}>
                          Configure mail server and email settings
                        </Card.Text>
                        <Button
